	...
	const copyCodeCommand = vscode.commands.registerCommand(
		...
		async (fileUri: vscode.Uri) => {
			...
			try {
				...
				const formattedContent = await getFormattedFileString(
					fileUri,
					workspaceFolder,
				);
				...
				vscode.window.showInformationMessage(
					`代码已复制: ${path.basename(fileUri.fsPath)}`,
				);
			} catch (error) {
				console.error(error);
		...
		async (folderUri: vscode.Uri) => {
			...
			try {
				...
				const allFormattedContents = await Promise.all(
					allFileUris.map((uri) =>
						getFormattedFileString(uri, workspaceFolder),
					),
				);
				...
				vscode.window.showInformationMessage(
					`已复制 ${folderName} 文件夹中 ${allFileUris.length} 个文件的内容。`,
				);
			} catch (error) {
				console.error(error);
